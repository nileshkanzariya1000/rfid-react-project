...
import { getUserSubjects } from "../service/api";
  ...
  return (
    <div className={`h-screen bg-black text-white p-4 ${isOpen ? "w-72" : "w-16"} flex flex-col transition-all duration-300`}>
      ...
      <ul className="space-y-3 flex-1">
        <li className={`flex items-center gap-2 cursor-pointer p-2 rounded ${selectedItem === "EditProfile" ? "bg-green-500 text-white" : "hover:text-green-500"}`} onClick={() => handleSelectItem("EditProfile")}>
          <PencilSquareIcon className="w-6 h-6" />
          {isOpen && <span>Edit Profile</span>}
        </li>

        <li className={`flex items-center gap-2 cursor-pointer p-2 rounded ${selectedItem === "EditPassword" ? "bg-green-500 text-white" : "hover:text-green-500"}`} onClick={() => handleSelectItem("EditPassword")}>
          <KeyIcon className="w-6 h-6" />
          {isOpen && <span>Edit Password</span>}
        </li>

        <li>
          <div className={`flex items-center justify-between cursor-pointer p-2 rounded ${selectedItem === "ViewSubject" ? "bg-green-500 text-white" : "hover:text-green-500"}`} onClick={() => setSubjectOpen(!subjectOpen)}>
        ...
        <li className={`flex items-center gap-2 cursor-pointer p-2 rounded ${selectedItem === "AddNewSubject" ? "bg-green-500 text-white" : "hover:text-green-500"}`} onClick={() => handleSelectItem("AddNewSubject")}>
      ...
      <div className="mb-4">
        <li className={`flex items-center gap-2 cursor-pointer p-2 rounded ${selectedItem === "Logout" ? "bg-red-500 text-white" : "hover:text-red-500"}`} onClick={() => handleSelectItem("Logout")}>
